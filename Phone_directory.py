...
def phone(strng, num):
    ...
    def get_addres(string, number):
        res = re.sub(r'<.+>', '', string)
        adr = re.sub(number, '', res)
        adress = re.sub(r'[;/\+\?\*\$]', '', adr)
        result = re.sub(r'_', ' ', adress)
        return ' '.join(result.lstrip().split())
    for i in range(0, len(string)):
        if num in string[i]:
            if len(re.findall(num, ''.join(string))) == 1:
                return 'Phone => {}, Name => {}, Address => {}'.format(num, str(re.findall(r'<.+>', string[i]))[3:-3], get_addres(string[i], num))
            elif len(re.findall(num, ''.join(string))) == 2:
